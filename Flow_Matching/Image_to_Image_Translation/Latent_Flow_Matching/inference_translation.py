...
# ---------------------------
# RK4 sampler in LATENT space (integrate t: 1 -> 0)
# ---------------------------
@torch.no_grad()
def rk4_generate_latent_with_grid(model, z_src: torch.Tensor, t_grid: torch.Tensor) -> torch.Tensor:
    """
    Integrate in latent space with conditioning.
    z_src: (B,4,H,W) latent of the source (scaled by scale_factor)
    t_grid: increasing grid from 0..1 (we integrate in reverse from 1->0)
    Returns latent at t=0.
    """
    device = z_src.device
    z = torch.randn_like(z_src)  # matches training where z1 ~ N(0,1) * scale_factor (z_src scale)
    assert t_grid.device == device

    B = z_src.size(0)
    tb = torch.empty(B, device=device, dtype=z.dtype)

    def f(t_s: float, z_s: torch.Tensor):
        tb.fill_(t_s)
        zin = torch.cat([z_s, z_src], dim=1)  # (B,8,H,W)
        return model(zin, tb, extra={})       # (B,4,H,W) velocity

    # integrate from 1 -> 0 using the provided 0..1 grid
    for i in range(t_grid.numel() - 1, 0, -1):
        t1 = float(t_grid[i].item())
        t0 = float(t_grid[i-1].item())
        h  = t0 - t1  # negative step (since t1>t0, we go downward)

        k1 = f(t1, z)
        k2 = f(t1 + 0.5*h, z + 0.5*h*k1)
        k3 = f(t1 + 0.5*h, z + 0.5*h*k2)
        k4 = f(t0,         z + h*k3)
...
# ---------------------------
# Dynamic generation with global progress (LATENT)
# ---------------------------
@torch.no_grad()
def generate_all_dynamic(
    ...
    while True:
        ...
        while idx < end:
            ...
            x_src = torch.stack(xs, dim=0).to(device, non_blocking=True)  # [-1,1]

            # ---- LATENT ENCODE ----
            with torch.no_grad():
                z_src = vae.encode(x_src).latent_dist.sample() * scale_factor  # (B,4,32,32)

            # ---- RK4 in LATENT ----
            z_T0 = rk4_generate_latent_with_grid(model, z_src, t_grid)

            # ---- DECODE to pixel ----
            gen_pm1 = vae.decode(z_T0 / scale_factor).sample  # [-1,1]
            gen01   = _pm1_to_01(gen_pm1)                     # [0,1]
            ...
            del x_src, z_src, z_T0, gen_pm1, gen01, gen01_cpu
            n_inc = jend - idx
...
def main(rank, world_size):
    ...
    # ----- Model (latent I/O): in=8 (z_t + z_src), out=4 (latent velocity) -----
    model = UNetModel(
        in_channels=8,
        model_channels=192,
        out_channels=4,
    ...
    # ----- Time grid (0..1), integrate 1->0 internally -----
