...
#In addition: we incorporated concatenated conditioning into the rectified-flow model by feeding the network both the current latent point zt 
# and the source latent z0, expanding the input from 4 to 8 channels to guide the flow with explicit source-domain information.”
...
from fid_eval_i2i_rectified_v2 import eval_fid_i2i_rectified
...
def main(rank, world_size, args):
    ...
    # NOTE: in_channels = 8
    model = UNetModel(
        in_channels=8,     # latent channels only
        model_channels=192,
    ...
    for epoch in range(args.epochs):
        ...
        for batch in pbar:
            ...
            x_input = torch.cat([z_t, z0], dim=1)  # (B, 8, 32, 32) 

            optimizer.zero_grad(set_to_none=True)
            with torch.cuda.amp.autocast():
                pred_vel = model(x_input, t, extra={})
