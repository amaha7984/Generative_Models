# Rectified Flow + DINOv3 NN pairing (bank) + DINO PatchNCE structural prior
# This setup builds a rectified path using input and target instances of images selected with DINOv3 embedding space and similarity in that space
# To preserve local structures two different patches are used one from input and another from generated image and processing with DINOv3
# then InfoNCE loss is computed between these patches
...
# ---------------------------------------------------------
# DINOv3 feature extractor (CLS + Patch tokens)
# ---------------------------------------------------------
class DINOv3FeatureExtractor(nn.Module):
    ...
    def forward_patches(self, x_pm1):
        x = self._prep(x_pm1)
        out = self.backbone.forward_features(x)
        if not (isinstance(out, dict) and "x_norm_patchtokens" in out):
            raise RuntimeError("DINO forward_features missing x_norm_patchtokens")
        return out["x_norm_patchtokens"]  # [B, N, D]


# ---------------------------------------------------------
# DINO PatchNCE loss (tokens_src vs tokens_gen)
# ---------------------------------------------------------
def dino_patch_nce_loss(tokens_src, tokens_gen, num_patches=32, temperature=0.07):
    """
    tokens_src: [B, N, D]
    tokens_gen: [B, N, D]
    positives: same patch index (sampled)
    negatives: other sampled patches across batch
    """
    B, N, D = tokens_src.shape
    device = tokens_src.device

    K = min(num_patches, N)
    idx = torch.randint(0, N, (B, K), device=device)  # [B, K]
    idx_e = idx.unsqueeze(-1).expand(-1, -1, D)       # [B, K, D]

    src = tokens_src.gather(1, idx_e).reshape(B * K, D)
    gen = tokens_gen.gather(1, idx_e).reshape(B * K, D)

    src = F.normalize(src, dim=-1)
    gen = F.normalize(gen, dim=-1)

    logits = (gen @ src.t()) / temperature           # [BK, BK]
...
def main(rank, world_size, args):
    ...
    for epoch in range(args.epochs):
        ...
        if epoch < args.struct_start_epoch:
            lambda_struct_eff = 0.0
        elif epoch < args.struct_start_epoch + args.struct_ramp_epochs:
            progress = (epoch - args.struct_start_epoch + 1) / float(args.struct_ramp_epochs)
            lambda_struct_eff = args.lambda_struct * progress
        else:
            lambda_struct_eff = args.lambda_struct

        if rank == 0:
            print(f"[Epoch {epoch+1}] lambda_struct_eff = {lambda_struct_eff:.4f}")
        ...
        for batch in pbar:
            ...
            with torch.cuda.amp.autocast():
                ...
                loss_fm = criterion(pred_vel, target_vel)

                # ---------------------------------------------------------
                # Structural loss: generate approximate x_gen and PatchNCE
                # ---------------------------------------------------------
                # one-step to t=1 from current (z_t, t)
                z_hat = z_t + (1.0 - t_b) * pred_vel
                x_gen = vae.decode(z_hat / scale_factor).sample   # [-1,1]

                # patch tokens
                tok_src = dino.forward_patches(x_src)             # [B,N,D]
                tok_gen = dino.forward_patches(x_gen)             # [B,N,D]

                if (not printed_shapes) and (rank == 0):
                    print("[DINO DEBUG] tok_src:", tok_src.shape, "tok_gen:", tok_gen.shape)
                    print("[BANK DEBUG] map_feats:", map_feats.shape, "map_latents:", map_latents.shape)
                    printed_shapes = True

                loss_struct = dino_patch_nce_loss(
                    tok_src, tok_gen,
                    num_patches=args.num_patches,
                    temperature=args.temperature,
                )

                loss = loss_fm + lambda_struct_eff * loss_struct
...
if __name__ == "__main__":
    ...
    # NEW: Map bank (CLS feats + latents)
    ...
    parser.add_argument("--lambda_struct", type=float, default=0.3,
                        help="Weight for DINO PatchNCE loss.")
    parser.add_argument("--struct_start_epoch", type=int, default=30,
                    help="Epoch to start ramping structural loss.")
    parser.add_argument("--struct_ramp_epochs", type=int, default=30,
                    help="Number of epochs to ramp lambda_struct.")

    parser.add_argument("--num_patches", type=int, default=32,
                        help="How many patch indices per image to sample for PatchNCE.")
