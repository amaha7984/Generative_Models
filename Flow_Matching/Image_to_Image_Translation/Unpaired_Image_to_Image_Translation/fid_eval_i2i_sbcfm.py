...
class EvalPairDataset(Dataset):
    def __init__(self, sat_dir, map_dir, size=256):
        ...
        self.sat_files = sorted([n for n in os.listdir(sat_dir) if n.lower().endswith((".png", ".jpg", ".jpeg"))])
        self.map_files = sorted([n for n in os.listdir(map_dir) if n.lower().endswith((".png", ".jpg", ".jpeg"))])
...
@torch.no_grad()
def _rk4_generate_latent_ode(model, z0, steps=200):
    """
    Generic ODE RK4 in latent space:
      dz/dt = v_theta(t, z)

    Works for SB-CFM, Rectified Flow, etc. as long as model returns drift.
    """
    device = z0.device
    z = z0.clone()
    B = z.shape[0]
...
@torch.no_grad()
def eval_fid_i2i_sbcfm(
    model, device,
    sat_dir, map_dir,
    out_dir, epoch,
    steps=200, batch_size=16, num_workers=4,
    ...
    if is_rank0 and save_samples > 0:
        sample_dir = os.path.join(out_dir, f"epoch_{epoch}_samples_sbcfm")
        ...
        for i, idx in enumerate(idxs):
            ...
            zT = _rk4_generate_latent_ode(model, z0, steps=steps)

            gen_pm1 = vae.decode(zT / scale_factor).sample
            ...
            del x0, x1, z0, zT, gen_pm1, gen01, vis

    total_local = len(sampler) if sampler is not None else len(ds)
    pbar = tqdm(
        total=total_local,
        desc=f"FID SB-CFM (epoch {epoch}) [rank {rank}/{world}]",
        ncols=100,
    ...
    for batch in dl:
        ...
        del real01, x_src, z0, zT, gen_pm1, gen01
    ...
    if is_rank0:
        log_path = os.path.join(out_dir, "fid_scores_sbcfm.txt")
