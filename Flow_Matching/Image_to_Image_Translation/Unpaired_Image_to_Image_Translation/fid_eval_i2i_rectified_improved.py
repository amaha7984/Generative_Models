# fid_eval_i2i_rectified_improved.py
# See file: train_latent_rectified_improved.py
...
def _rk4_generate_latent_rectified(model, z0, steps=50):
    ...
    z0:   (B,4,H,W) latent of the source (e.g., satellite), scaled by scale_factor.
...
@torch.no_grad()
def eval_fid_i2i_rectified_rfpp(
    model, device,
    sat_dir, map_dir,
    ...
    sampler = DistributedSampler(ds, num_replicas=world, rank=rank,
                                 shuffle=False, drop_last=False) if is_dist else None
    dl = DataLoader(ds, batch_size=batch_size, shuffle=False, sampler=sampler,
                    num_workers=num_workers, pin_memory=True, drop_last=False)

    fid = FrechetInceptionDistance(normalize=True).to(device)

    # Save a few visual samples using latent RK4 path
    ...
    for batch in dl:
        real01 = batch["map_01"].to(device, non_blocking=True)  # [0,1], real targets
    ...
    if is_rank0:
        log_path = os.path.join(out_dir, "fid_scores_rectified_rfpp.txt")
